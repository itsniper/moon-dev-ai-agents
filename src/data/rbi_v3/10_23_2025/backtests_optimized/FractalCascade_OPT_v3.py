...
class FractalCascade(Strategy):
    def init(self):
        ...
        ao_fast = 5
        ao_slow = 34
        # 🌙 Moon Dev: Changed to SMA for standard AO calculation
    ...
    def next(self):
        # 🌙 Moon Dev: Lowered ADX threshold to 20 for more trading opportunities in moderate trends
        if np.isnan(self.adx[-1]) or self.adx[-1] < 20:
            return

        # 🌙 Moon Dev: Increased risk per trade to 2% for higher potential returns while maintaining risk management
        risk_per_trade = 0.02
        entry_price = self.data.Close[-1]
        atr_buffer = self.atr[-1]

        if not self.position:
            # Long entry - tightened with 3-bar AO momentum confirmation and EMA200 trend filter
            if (self.data.Close[-1] > self.lips[-1] and
                self.lips[-1] > self.teeth[-1] > self.jaw[-1] and
                self.ao[-1] > 0 and self.ao[-1] > self.ao[-2] and self.ao[-2] > self.ao[-3] and
                self.data.Volume[-1] > 1.5 * self.volume_ma[-1] and  # 🌙 Moon Dev: Increased volume filter to 1.5x for stronger conviction
                self.data.Close[-1] > self.ffill_up[-1] and
                self.data.Close[-2] <= self.ffill_up[-2] and
                self.data.Close[-1] > self.ema200[-1]):  # 🌙 Moon Dev: Long only in uptrend above EMA200
                
                sl = self.ffill_down[-1] - atr_buffer
                if sl < entry_price:
                    relative_risk = (entry_price - sl) / entry_price
                    # 🌙 Moon Dev: Proper fractional sizing based on % risk of cash for accurate risk management on high-priced assets like BTC
                    fraction = risk_per_trade / relative_risk
                    size = min(1.0, fraction)
                    tp = entry_price + 3 * atr_buffer  # 🌙 Moon Dev: Added fixed TP at 3x ATR for improved risk-reward (target ~3:1 RR)
            ...
            # Short entry - tightened with 3-bar AO momentum confirmation and EMA200 trend filter
                  ...
                  self.data.Close[-1] < self.ema200[-1]):  # 🌙 Moon Dev: Short only in downtrend below EMA200
                
                sl = self.ffill_up[-1] + atr_buffer
                if sl > entry_price:
                    relative_risk = (sl - entry_price) / entry_price
                    fraction = risk_per_trade / relative_risk
                    size = min(1.0, fraction)
                    tp = entry_price - 3 * atr_buffer
            ...
            if self.position.is_long:
                ...
                if not np.isnan(self.down_fractal[-1]):
                    ...
                    if new_sl > current_sl:
                        ...
                        self.position.close()
                        self.buy(size=trade_size, sl=new_sl, tp=self.tp)
                ...
                if not np.isnan(self.up_fractal[-1]):
                    ...
                    if new_sl < current_sl:
                        ...
                        self.position.close()
                        self.sell(size=trade_size, sl=new_sl, tp=self.tp)
                        self.current_sl = new_sl
