                ...
                if len(low_slice) == 2 * self.strength + 1:
                    ...
                    if low_slice.iloc[pivot_rel_idx] == low_slice.min():
                        ...
                        if not self.low_pivots or self.low_pivots[-1][0] != pivot_i:
                            ...
                            if len(self.low_pivots) >= 2 and self.data.Close.iloc[-1] > self.sma[-1]:
                                ...
                                if curr_low < prev_low and curr_rsi > prev_rsi:
                                    ...
                                    if high_start < high_end and high_end <= current_bar + 1:
                                        swing_high_slice = self.data.High.iloc[high_start:high_end]
                ...
                if len(high_slice) == 2 * self.strength + 1:
                    ...
                    if high_slice.iloc[pivot_rel_idx] == high_slice.max():
                        ...
                        if not self.high_pivots or self.high_pivots[-1][0] != pivot_i:
                            ...
                            if len(self.high_pivots) >= 2 and self.data.Close.iloc[-1] < self.sma[-1]:
                                ...
                                if curr_high > prev_high and curr_rsi < prev_rsi:
                                    ...
                                    if low_start < low_end and low_end <= current_bar + 1:
                                        swing_low_slice = self.data.Low.iloc[low_start:low_end]
...
bt = Backtest(data, FibonacciDivergence, cash=1000000, commission=0.002, exclusive_orders=True)
stats = bt.run()
print(stats)
