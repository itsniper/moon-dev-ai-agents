...
class AdaptiveDivergence(Strategy):
    pivot_length = 5  # 🌙 Kept at 5 for reliable swings, avoids noise
    rsi_div_thresh = 2  # 🌙 Lowered to 2 for more divergence signals without overtrading
    mom_long_thresh = 45  # 🌙 Adjusted to 45 to catch improving momentum earlier in bullish setups
    mom_short_thresh = 55  # 🌙 Adjusted to 55 for bearish setups, symmetric improvement
    risk_per_trade = 0.01  # 🌙 Kept at 1% for solid risk management
    sl_mult = 1.5  # 🌙 Maintained for balanced stop placement
    tp_mult = 3.0  # 🌙 Increased to 3x ATR for 1:2 R:R ratio, aiming for higher returns
    trail_mult = 2.0  # 🌙 Tightened trailing to 2x ATR to lock in profits better
    profit_trail = 1.5  # 🌙 Raised activation threshold to 1.5x ATR to avoid premature trailing
    max_bars = 20  # 🌙 Reduced to 20 bars for faster exits in non-trending moves
    atr_avg_period = 14  # 🌙 Shortened to 14 for more responsive volatility averaging
    ...
    def init(self):
        ...
        self.ema = self.I(talib.EMA, self.data.Close, timeperiod=50)  # 🌙 Changed to 50-period EMA for faster trend detection in crypto volatility
        self.adx = self.I(talib.ADX, self.data.High, self.data.Low, self.data.Close, timeperiod=14)  # 🌙 Added ADX for trend strength filter (>25 only)
        self.vol_sma = self.I(talib.SMA, self.data.Volume, timeperiod=20)  # 🌙 Added volume SMA for confirmation of strong moves

        self.confirmed_swing_lows = deque(maxlen=5)  # 🌙 Reduced maxlen to 5 to focus on recent swings
        self.confirmed_swing_highs = deque(maxlen=5)
        self.last_checked = -1
    ...
    def next(self):
        ...
        if current_bar < 50:  # 🌙 Adjusted wait period to match shorter EMA
            return
        ...
        if self.position.size != 0:
            ...
            if self.is_long:
                ...
                # Reversal check - 🌙 Adjusted thresholds for earlier reversal detection
        ...
        # Entry logic if no position
            ...
            # 🌙 Loosened volatility filter to >0.5x average for more trading opportunities in moderate vol
            ...
            # 🌙 Added ADX and volume filters for higher quality entries
            ...
            if (bullish_div and self.current_mom > self.mom_long_thresh and mom_rising and above_ema and strong_trend and strong_volume):
                ...
                vol_mult = 0.5 if self.atr[-1] > 1.5 * self.atr_avg[-1] else 1.0  # 🌙 Kept volatility adjustment for risk control
    ...
    def detect_bullish_div(self):
        ...
        if current_bar - last[0] > 30:  # 🌙 Tightened to 30 bars for fresher signals
            return False
