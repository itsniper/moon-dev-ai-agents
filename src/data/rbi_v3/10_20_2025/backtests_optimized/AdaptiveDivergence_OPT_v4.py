...
class AdaptiveDivergence(Strategy):
    pivot_length = 3  # 🌙 Reduced to 3 for more frequent swing detections, increasing divergence opportunities without excessive noise
    rsi_div_thresh = 3  # 🌙 Increased to 3 for stronger, higher-quality divergence signals to filter out weak setups
    mom_long_thresh = 50  # 🌙 Raised to 50 to enter only on stronger bullish momentum, improving win rate
    mom_short_thresh = 50  # 🌙 Adjusted to 50 for symmetric bearish threshold, ensuring balanced and selective shorts
    risk_per_trade = 0.02  # 🌙 Increased to 2% risk per trade for more aggressive capital deployment to chase higher returns
    sl_mult = 1.0  # 🌙 Tightened to 1x ATR for closer stops, reducing losses and enabling better risk-reward
    tp_mult = 4.0  # 🌙 Boosted to 4x ATR for 1:4 R:R ratio, capturing larger moves to drive returns toward 50% target
    trail_mult = 1.5  # 🌙 Tightened to 1.5x ATR for quicker profit locking, protecting gains in volatile crypto swings
    profit_trail = 2.0  # 🌙 Raised activation to 2x ATR to allow more room before trailing, avoiding early exits in trends
    max_bars = 50  # 🌙 Extended to 50 bars for longer holds in strong trends, giving positions more time to reach TP
    atr_avg_period = 14  # 🌙 Kept at 14 for responsive volatility, no change needed
    ...
    def init(self):
        ...
        self.ema = self.I(talib.EMA, self.data.Close, timeperiod=21)  # 🌙 Adjusted to 21-period EMA, a Fibonacci number for smoother trend filter in crypto
        self.adx = self.I(talib.ADX, self.data.High, self.data.Low, self.data.Close, timeperiod=14)  # 🌙 Kept ADX but will use lower threshold in entries for more opportunities
        self.vol_sma = self.I(talib.SMA, self.data.Volume, timeperiod=20)  # 🌙 Kept volume SMA

        self.confirmed_swing_lows = deque(maxlen=3)  # 🌙 Reduced to 3 for focus on most recent swings, fresher divergences
    ...
    def next(self):
        ...
        if current_bar < 21:  # 🌙 Reduced wait to match shorter EMA initialization
            return
        ...
        if self.position.size != 0:
            ...
            if self.is_long:
                ...
                # Reversal check - 🌙 Tightened thresholds for quicker reversal exits to preserve capital
                if self.detect_bearish_div() or self.current_mom < 35:
                    self.position.close()
                ...
                # Reversal check - 🌙 Tightened thresholds for quicker reversal exits to preserve capital
                if self.detect_bullish_div() or self.current_mom > 65:
                    self.position.close()
        ...
        # Entry logic if no position
            ...
            # 🌙 Loosened volatility filter to >0.3x average for more entry opportunities while still avoiding dead markets
            if np.isnan(self.atr[-1]) or np.isnan(self.atr_avg[-1]) or self.atr[-1] <= 0.3 * self.atr_avg[-1]:
                print(f"🌙 Low volatility filter: ATR {self.atr[-1]:.2f} <= 0.3*Avg {self.atr_avg[-1]:.2f} 😴")
                return

            above_ema = self.data.Close[-1] > self.ema[-1]
            below_ema = not above_ema

            # 🌙 Loosened ADX to >20 and volume to >1.0x SMA for more trades in moderate trends, balancing quality and frequency
            strong_trend = self.adx[-1] > 20
            strong_volume = self.data.Volume[-1] > 1.0 * self.vol_sma[-1]
            ...
            if (bullish_div and self.current_mom > self.mom_long_thresh and mom_rising and above_ema and strong_trend and strong_volume):
                ...
                vol_mult = 0.3 if self.atr[-1] > 1.5 * self.atr_avg[-1] else 1.0  # 🌙 Tightened vol_mult to 0.3 in high vol for stricter sizing, better risk control
            ...
            # Short entry
                ...
                raw_size = risk_amount / sl_distance
                cash_size = self._broker._cash / self.data.Close[-1]
                vol_mult = 0.3 if self.atr[-1] > 1.5 * self.atr_avg[-1] else 1.0
                size = min(cash_size, raw_size) * vol_mult
                size = max(1, int(round(size)))
                self.sell(size=size)
                self.entry_price = self.data.Close[-1]
    ...
    def detect_bullish_div(self):
        ...
        if current_bar - last[0] > 20:  # 🌙 Reduced to 20 bars for even fresher, more timely divergence signals
            return False
