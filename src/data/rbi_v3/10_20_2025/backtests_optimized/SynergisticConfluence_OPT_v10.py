...
class SynergisticConfluence(Strategy):
    ...
    swing_period = 10
    avg_atr_period = 20
    ema_period = 200  # 🌙 Moon Dev: Added long-term EMA for trend regime filter
    vol_sma_period = 20  # 🌙 Moon Dev: Added volume SMA for spike confirmation
    risk_per_trade = 0.015  # 🌙 Moon Dev: Increased risk per trade from 0.01 to 0.015 for higher returns with controlled risk
    max_bars_held = 30  # 🌙 Moon Dev: Extended max hold from 20 to 30 bars (7.5 hours on 15m) to allow more room for trends
    rr_tp = 3  # 🌙 Moon Dev: Set TP to 3:1 RR for better reward potential
    rr_partial = 1  # 🌙 Moon Dev: Partial profit at 1:1 RR to scale out while letting winners run
    trail_atr_mult = 1.5  # 🌙 Moon Dev: Trailing stop multiplier
    sl_atr_mult = 1.5  # 🌙 Moon Dev: Tightened initial SL to 1.5 ATR below/above swing (from 2) for better risk-reward
    vol_spike_mult = 1.2  # 🌙 Moon Dev: Volume spike threshold
    adx_threshold = 25  # 🌙 Moon Dev: Tightened ADX threshold from 20 to 25 for stronger trend confirmation

    def init(self):
        close = self.data.Close
        high = self.data.High
        low = self.data.Low
        volume = self.data.Volume  # 🌙 Moon Dev: Use volume for entry filter
        ...
        # 🌙 Moon Dev: Added EMA200 for overall trend filter to avoid counter-trend trades
        self.ema200 = self.I(talib.EMA, close, timeperiod=self.ema_period)
        # 🌙 Moon Dev: Added volume SMA for spike filter to confirm momentum on entries
    ...
    def next(self):
        if len(self.data) < self.ema_period:  # 🌙 Moon Dev: Increased warmup to 200 bars for EMA200 initialization
            return
        ...
        vol_spike = current_vol > current_vol_sma * self.vol_spike_mult  # 🌙 Moon Dev: Volume spike filter for high-conviction entries

        # 🌙 Moon Dev: Enhanced filters - tighter ADX for stronger trends, kept volatility filter
        vol_filter = current_atr > current_avg_atr
        trend_filter = current_adx > self.adx_threshold
        ...
        if self.position:
            ...
            # 🌙 Moon Dev: Improved exit logic - check TP before partial, fixed partial to 50% of current position, use RR levels instead of BB/RSI
            if self.is_long:
                partial_level = self.entry_price + self.initial_risk
                ...
                if current_low <= self.stop_price:
                    ...
                    return
                # Full TP check (3:1 RR)
                if current_close >= self.tp_price:
                    self.position.close()
                    print(f"🌙 Moon Dev Full TP Long at {current_close} 🚀")
                    return
                # Partial profit at 1:1 RR
                if current_close >= partial_level and not self.partial_taken:
        ...
        # Long entry - added EMA200 > close and volume spike for better trend alignment and momentum confirmation
        ...
        long_trend = current_close > current_ema200
        if long_rsi_cross and long_bb_touch and long_macd_cross and long_hist_expand and vol_spike and long_trend:
            if self.position and not self.is_long:
                self.position.close()
            stop_price = current_swing_low - self.sl_atr_mult * current_atr  # 🌙 Moon Dev: Tightened initial SL
            risk_distance = current_close - stop_price
            if risk_distance > 0:
                risk_amount = self.equity * self.risk_per_trade
                size = risk_amount / risk_distance  # 🌙 Moon Dev: Use float size for precise position sizing (no int rounding)
                self.buy(size=size)
                self.entry_price = current_close
                self.stop_price = stop_price
                self.initial_stop = stop_price
                self.initial_risk = risk_distance
                self.tp_price = self.entry_price + self.rr_tp * self.initial_risk  # 🌙 Moon Dev: Set initial TP to 3:1 RR
                self.partial_taken = False
                self.entry_bar = len(self.data)
                self.is_long = True
                print(f"🌙 Moon Dev LONG Entry at {current_close}, Size: {size}, SL: {stop_price}, TP: {self.tp_price} 🚀")

        # Short entry - added EMA200 < close and volume spike for better trend alignment and momentum confirmation
        ...
        short_trend = current_close < current_ema200
        if short_rsi_cross and short_bb_touch and short_macd_cross and short_hist_expand and vol_spike and short_trend:
            if self.position and self.is_long:
                self.position.close()
            stop_price = current_swing_high + self.sl_atr_mult * current_atr  # 🌙 Moon Dev: Tightened initial SL
