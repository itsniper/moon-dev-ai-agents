...
class ExtremeDivergence(Strategy):
    def init(self):
        # 🌙 Moon Dev Optimization: Switched to EMA 100 for faster trend detection to catch more opportunities
        self.ema = self.I(talib.EMA, self.data.Close, timeperiod=100)
        # 🌙 Moon Dev Optimization: Added ADX for trend strength filter to avoid choppy markets
        self.adx = self.I(talib.ADX, self.data.High, self.data.Low, self.data.Close, timeperiod=14)
        # 🌙 Moon Dev Optimization: Added volume MA for confirmation of momentum on entry
        self.vol_ma = self.I(talib.SMA, self.data.Volume, timeperiod=5)
        ...
        self.entry_price = 0
        self.trail_sl = 0
        self.tp = 0  # 🌙 Moon Dev Optimization: Added take profit level
        self.highest = 0
        self.lowest = 0

    def next(self):
        current_len = len(self.ema)
        if current_len < 11:
            return
        # 🌙 Moon Dev Optimization: Updated trend detection with EMA100, slope over 10 periods, and ADX > 20 for stronger trends
        ema_slope = self.ema[-1] - self.ema[-11]
        uptrend = self.data.Close[-1] > self.ema[-1] and ema_slope > 0 and self.adx[-1] > 20
        downtrend = self.data.Close[-1] < self.ema[-1] and ema_slope < 0 and self.adx[-1] > 20

        # 🌙 Moon Dev Optimization: Increased order to 10 for more significant swing detection, min distance 10 bars for quality setups
        # Compute bullish hidden divergence
        bullish_div = False
        recent_swing_low = 0
        price_slice_low = self.data.Low[:current_len]
        low_indices = argrelextrema(price_slice_low, np.less_equal, order=10)[0]
        ...
        high_indices = argrelextrema(price_slice_high, np.greater_equal, order=10)[0]
        ...
        if not self.position:
            # 🌙 Moon Dev Optimization: Added momentum confirmation (hist rising/falling, MACD direction, volume > MA) and relaxed RSI to 35/65 for more entries
            if (uptrend and bullish_div and self.rsi[-1] < 35 and
                self.hist[-1] > self.hist[-2] and self.macd_line[-1] > self.signal_line[-1] and
                ...
                risk_dist = entry - sl
                if risk_dist > 0:
                    # 🌙 Moon Dev Optimization: Allow fractional sizing for precise risk management (1% risk)
                    size = (self.equity * 0.01) / risk_dist
                    if size > 0:
                        self.buy(size=size)
                        self.entry_price = entry
                        self.trail_sl = sl
                        self.tp = entry + 4 * risk_dist  # 🌙 Moon Dev Optimization: Added 4:1 RR take profit for higher returns
                        self.highest = self.data.High[-1]
                        print(f"🌙 Moon Dev Long Entry at {entry:.2f}! Size: {size}, SL: {sl:.2f}, TP: {self.tp:.2f} ✨🚀")
            elif (downtrend and bearish_div and self.rsi[-1] > 65 and
                  self.hist[-1] < self.hist[-2] and self.macd_line[-1] < self.signal_line[-1] and
                  self.data.Volume[-1] > self.vol_ma[-1]):
                entry = self.data.Close[-1]
                atr_val = self.atr[-1]
                ...
                if risk_dist > 0:
                    ...
                    if size > 0:
                        ...
                        self.entry_price = entry
                        self.trail_sl = sl
                        self.tp = entry - 4 * risk_dist  # 🌙 Moon Dev Optimization: Added 4:1 RR take profit
