...
class FibonacciDivergence(Strategy):
    sma_period = 50  # 🌙 Moon Dev: Reduced SMA period from 200 to 50 for faster trend response on 15m timeframe, capturing shorter-term trends more effectively
    rsi_period = 9   # 🌙 Moon Dev: Shortened RSI period from 14 to 9 for increased sensitivity to momentum divergences without excessive noise
    atr_period = 14
    adx_period = 14  # 🌙 Moon Dev: Added ADX indicator for trend strength filter to avoid entries in weak/choppy markets
    adx_threshold = 25  # 🌙 Moon Dev: Only enter trades when ADX > 25, ensuring stronger trending conditions
    vol_sma_period = 20  # 🌙 Moon Dev: Added volume SMA for confirmation filter
    vol_multiplier = 1.5  # 🌙 Moon Dev: Require volume > 1.5x SMA for higher conviction entries
    strength = 7     # 🌙 Moon Dev: Increased pivot strength from 5 to 7 for more reliable pivot detection, reducing false signals
    risk_pct = 0.02  # 🌙 Moon Dev: Increased risk per trade from 1% to 2% to amplify returns while still managing risk
    key_fib_levels = [0.0, 0.236, 0.382, 0.5, 0.618, 0.786, 1.0]  # 🌙 Moon Dev: Focused on core retracement levels, removed extensions for more precise pullback entries
    fib_tolerance = 0.02  # 🌙 Moon Dev: Tightened Fibonacci tolerance from 0.05 to 0.02 for higher-quality setups at key levels
    min_fib_range = 0.005  # 🌙 Moon Dev: Added minimum Fibonacci range (0.5% of price) to filter out insignificant swings
    ...
    def init(self):
        ...
        self.adx = self.I(talib.ADX, self.data.High, self.data.Low, self.data.Close, timeperiod=self.adx_period)  # 🌙 Moon Dev: Initialized ADX for market regime filtering
        self.vol_sma = self.I(talib.SMA, self.data.Volume, timeperiod=self.vol_sma_period)  # 🌙 Moon Dev: Initialized volume SMA for entry confirmation
        self.low_pivots = []
        self.high_pivots = []
                            ...
                            if len(self.low_pivots) >= 2 and self.data.Close[current_bar] > self.sma[current_bar]:
                                ...
                                if curr_low < prev_low and curr_rsi > prev_rsi:
                                    ...
                                    if high_start < high_end and high_end <= current_bar + 1:
                                        ...
                                        # 🌙 Moon Dev: Added minimum fib_range filter to avoid tiny swings
                                        if fib_range > self.data.Close[current_bar] * self.min_fib_range:
                                            ...
                                            # 🌙 Moon Dev: Added ADX and volume filters for better entry quality
                                            if (is_at_fib and 
                                                self.adx[current_bar] > self.adx_threshold and 
                                                self.data.Volume[current_bar] > self.vol_multiplier * self.vol_sma[current_bar]):
                                                ...
                                                if atr_val > 0:
                                                    ...
                                                    # 🌙 Moon Dev: Improved RR from 1:2 to 1:3 for higher reward potential
