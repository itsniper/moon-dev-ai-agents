...
class DivergentConvergence(Strategy):
    ...
    lookback = 20  # 🌙 Moon Dev Optimization: Extended lookback to 20 for broader historical low detection, capturing stronger potential divergences
    recent_period = 10  # 🌙 Moon Dev Optimization: Increased recent period to 10 for more stable recent low identification, reducing noise
    stoch_window = 5  # 🌙 Moon Dev Optimization: Reduced stochastic window to 5 for faster, more responsive convergence signals in volatile BTC
    risk_pct = 0.02  # 🌙 Moon Dev Optimization: Increased risk per trade to 2% to deploy more capital aggressively while scaling towards 50% target
    rr = 3.0  # 🌙 Moon Dev Optimization: Boosted RR to 3:1 to amplify reward capture on winning trades, balancing higher returns with risk
    max_bars = 50  # 🌙 Moon Dev Optimization: Further extended max hold time to 50 bars to give emerging trends more development space
    atr_mult_sl = 1.5  # 🌙 Moon Dev Optimization: Tightened ATR multiplier for trailing stop to 1.5x, locking in profits more proactively
    buffer_mult = 0.5  # 🌙 Moon Dev Optimization: Reduced buffer multiplier to 0.5x for tighter initial SL, improving risk-reward from entry

    def init(self):
        self.rsi = self.I(talib.RSI, self.data.Close, timeperiod=14)
        self.slowk = self.I(lambda h, l, c: talib.STOCH(h, l, c, fastk_period=8, slowk_period=3, slowd_period=3)[0], self.data.High, self.data.Low, self.data.Close)  # 🌙 Moon Dev Optimization: Switched to faster STOCH(8,3,3) for quicker oversold recovery signals
        ...
        # 🌙 Moon Dev Optimization: Added EMA20 for short-term momentum confirmation to filter entries in uptrending micro-structures
    ...
    def next(self):
        ...
        bull_div = (low2 <= low1) and (rsi2 > rsi1) and (rsi2 < 40) and (distance <= 5)  # 🌙 Moon Dev Optimization: Relaxed price condition to <= for more inclusive divergences, deepened RSI to <40, extended distance to <=5 for timely yet robust signals
        if bull_div:
            self.div_low = low2
        ...
        stoch_converge = (k_now < 20) and (k_now > k_prev) and (gap_now < gap_prev) and (k_now < d_now)  # 🌙 Moon Dev Optimization: Tightened %K threshold to <20 for stronger oversold convergence in BTC's volatile swings
        if stoch_converge:
            print(f"🚀 Moon Dev: Stochastic Convergence! %K: {k_now}, %D: {d_now}, Gap narrowed! 🌙")

        # Entry Logic
        if not self.position and self.div_bar is not None and stoch_converge and current_price > self.sma200[-1] and current_price > self.ema20[-1] and self.data.Volume[-1] > self.vol_sma[-1] * 1.5 and self.adx[-1] > 25:  # 🌙 Moon Dev Optimization: Added EMA20 momentum filter, strengthened volume to >1.5x SMA, and ADX to >25 for high-conviction entries in strong uptrends only
        ...
        if self.position:
            ...
            # RSI overbought exit
            if rsi_now > 75:  # 🌙 Moon Dev Optimization: Raised RSI exit threshold to >75 to allow more room for winners before overbought exit
                self.position.close()
