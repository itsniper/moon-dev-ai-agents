...
class DivergentConvergence(Strategy):
    ...
    lookback = 20  # 🌙 Moon Dev Optimization: Extended lookback to 20 for detecting stronger, more significant older lows in volatile BTC markets
    recent_period = 10  # 🌙 Moon Dev Optimization: Increased recent period to 10 to capture fresher divergences while filtering out excessive noise
    stoch_window = 14  # 🌙 Moon Dev Optimization: Aligned stochastic window with standard RSI period for better synchronization and smoother signals
    risk_pct = 0.02  # 🌙 Moon Dev Optimization: Raised risk per trade to 2% to increase exposure and potential returns, balanced with robust SL
    rr = 3.0  # 🌙 Moon Dev Optimization: Increased RR target to 3:1 mentally for aiming at larger profits, though relying on trailing for execution
    max_bars = 40  # 🌙 Moon Dev Optimization: Extended maximum hold time to 40 bars to allow BTC trends more development space without early exits
    atr_mult_sl = 2.5  # 🌙 Moon Dev Optimization: Loosened ATR multiplier to 2.5 for trailing stop to give positions more breathing room in high-volatility conditions
    buffer_mult = 1.0  # 🌙 Moon Dev Optimization: Increased buffer multiplier to 1.0 ATR below div low for wider initial SL, reducing wick-induced stops
    ...
    def init(self):
        ...
        # 🌙 Moon Dev Optimization: Added EMA20 for short-term momentum confirmation to ensure entry aligns with recent uptrend
        self.ema20 = self.I(talib.EMA, self.data.Close, timeperiod=20)
        self.entry_bar = None
        self.entry_price = None
        self.sl_price = None
        # 🌙 Moon Dev Optimization: Removed tp_price as we're ditching fixed TP to let winners run via trailing SL
        self.div_low = None
        self.div_bar = None
    ...
    def next(self):
        ...
        bull_div = (low2 < low1) and (rsi2 > rsi1) and (rsi2 < 40) and (distance <= 5)  # 🌙 Moon Dev Optimization: Tightened RSI oversold threshold to <40 for deeper, higher-quality divergences and extended distance to <=5 for timely but not too immediate signals
        if bull_div:
            self.div_low = low2
        ...
        # Entry Logic
        if not self.position and self.div_bar is not None and stoch_converge and current_price > self.sma200[-1] and current_price > self.ema20[-1] and self.data.Volume[-1] > 1.2 * self.vol_sma[-1] and self.adx[-1] > 18:  # 🌙 Moon Dev Optimization: Added EMA20 momentum filter for short-term uptrend confirmation; adjusted volume to >1.2x SMA for moderately higher conviction; lowered ADX to >18 for more trending opportunities without excessive chop
            ...
            if size > 0:
                # 🌙 Moon Dev Optimization: Calculate mental TP for reference but removed fixed TP from buy order to enable full trailing management and capture larger BTC moves
                ...
                self.div_bar = None
                self.div_low = None
                print(f"🌙 Moon Dev: LONG ENTRY! Size: {size} (fraction), Entry: {entry_price}, Initial SL: {sl_price}, Mental TP: {tp_price} 🚀✨")
            else:
        ...
        if self.position:
            ...
            # 🌙 Moon Dev Optimization: Update trailing SL dynamically every bar to lock in profits as price rises, but only tighten after breaking even to avoid premature adjustments
            ...
            # RSI overbought exit
            if rsi_now > 65:  # 🌙 Moon Dev Optimization: Lowered overbought threshold to >65 for earlier profit-taking in momentum shifts
                self.position.close()
                print(f"🌙 Moon Dev: RSI Overbought >65! EXIT 📈")
                self.current_sl = None
                return
