...
class ExtremeDivergence(Strategy):
    def init(self):
        ...
        # 🌙 Moon Dev Optimization: Added ADX for trend strength filter to avoid choppy markets and only trade in favorable trending conditions
        self.adx = self.I(talib.ADX, self.data.High, self.data.Low, self.data.Close, timeperiod=14)
        # 🌙 Moon Dev Optimization: Added volume SMA for confirmation filter to ensure entries on above-average volume for higher quality signals
        self.vol_sma = self.I(talib.SMA, self.data.Volume, timeperiod=20)
        self.entry_price = 0
        self.risk_dist = 0  # 🌙 Moon Dev Optimization: Track initial risk distance for dynamic TP calculation
        self.tp = 0  # 🌙 Moon Dev Optimization: Take Profit level for 3:1 reward-to-risk ratio to improve returns
        self.trail_sl = 0
        self.highest = 0
        self.lowest = 0
    ...
    def next(self):
        ...
        if len(low_indices) >= 2:
            idx1 = low_indices[-2]
            idx2 = low_indices[-1]
            # 🌙 Moon Dev Optimization: Added recency filter (within last 100 bars) to focus on fresh divergences and avoid stale signals
            if idx2 - idx1 >= 5 and idx1 > current_len - 100:
        ...
        if not self.position:
            if uptrend and bullish_div and self.rsi[-1] < 40 and self.adx[-1] > 25 and self.data.Volume[-1] > self.vol_sma[-1]:
                # 🌙 Moon Dev Optimization: Loosened RSI to <40 for more entry opportunities while maintaining oversold filter; added ADX>25 and volume> SMA for better setups
                entry = self.data.Close[-1]
                atr_val = self.atr[-1]
                sl = recent_swing_low - 1.5 * atr_val
                risk_dist = entry - sl
                if risk_dist > 0:
                    # 🌙 Moon Dev Optimization: Increased risk to 2% per trade for higher potential returns; use float size for precise positioning
                    size = (self.equity * 0.02) / risk_dist
                    if size > 0:
                        self.buy(size=size)
                        self.entry_price = entry
                        self.risk_dist = risk_dist
                        self.tp = entry + 3 * risk_dist  # 🌙 Moon Dev Optimization: Set 3:1 RR TP for improved reward capture
                        self.trail_sl = sl
                        self.highest = self.data.High[-1]
                        print(f"🌙 Moon Dev Long Entry at {entry:.2f}! Size: {size:.4f}, SL: {sl:.2f} ✨🚀")
            elif downtrend and bearish_div and self.rsi[-1] > 60 and self.adx[-1] > 25 and self.data.Volume[-1] > self.vol_sma[-1]:
                # 🌙 Moon Dev Optimization: Loosened RSI to >60 for more entry opportunities while maintaining overbought filter; added ADX>25 and volume> SMA for better setups
                entry = self.data.Close[-1]
                atr_val = self.atr[-1]
                ...
                if risk_dist > 0:
                    ...
                    if size > 0:
                        ...
                        print(f"🌙 Moon Dev Short Entry at {entry:.2f}! Size: {size:.4f}, SL: {sl:.2f} ✨📉")
        ...
        if self.position:
            if self.position.is_long:
                ...
                exit_reason = None
                # 🌙 Moon Dev Optimization: Prioritized exit conditions with TP first, then SL, RSI, EMA for better profit locking and risk control
