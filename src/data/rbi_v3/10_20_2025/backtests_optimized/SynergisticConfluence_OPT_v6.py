...
class SynergisticConfluence(Strategy):
    ...
    swing_period = 10
    avg_atr_period = 20
    ema_period = 200  # New: Added EMA for overall trend filter
    vol_ma_period = 20  # New: Added volume moving average for spike filter
    risk_per_trade = 0.02  # Increased from 0.01 to 0.02 for higher exposure and potential returns, while still managing risk
    max_bars_held = 50  # Increased from 20 to 50 (about 12.5 hours on 15m) to allow trades more time to develop
    rr_ratio = 3  # New: Improved RR to 3:1 for better profit potential
    partial_rr = 1  # New: Take partial at 1R instead of BB/RSI for more reliable scaling

    def init(self):
        close = self.data.Close
        high = self.data.High
        low = self.data.Low
        volume = self.data.Volume  # New: Use volume for spike filter
        ...
        # New: EMA for trend filter to avoid counter-trend trades
        self.ema = self.I(talib.EMA, close, timeperiod=self.ema_period)
        # New: Volume MA for spike confirmation to filter low-quality entries
        ...
        self.entry_bar = 0
        self.is_long = False
        self.risk_distance = 0  # New: Track risk distance for partial and TP calculations
    ...
    def next(self):
        ...
        current_ema = self.ema[-1]  # New: EMA for trend
        current_vol = self.data.Volume[-1]  # New: Current volume
        current_vol_ma = self.vol_ma[-1]  # New: Volume MA

        # Volatility and trend filter (kept ADX at 20 for more opportunities, added EMA trend)
        vol_filter = current_atr > current_avg_atr
        trend_filter = current_adx > 20
        # New: Volume spike filter for better entry quality
        vol_spike = current_vol > current_vol_ma * 1.2
        ...
        if self.position:
            ...
            # Update trailing stop (tightened to 1.2 ATR for better risk control after partial)
            trail_multiplier = 1.2 if self.partial_taken else 1.5
            if self.is_long:
                trail_sl = current_close - trail_multiplier * current_atr
                ...
                if current_low <= self.stop_price:
                    ...
                    return
                # Improved partial: at 1R profit instead of BB/RSI for more consistent scaling
                profit_distance = current_close - self.entry_price
                partial_cond = profit_distance >= self.partial_rr * self.risk_distance and not self.partial_taken
                if partial_cond:
                    self.sell(size=0.5)
                    self.partial_taken = True
                    print(f"🌙 Moon Dev Partial Profit Long at {current_close} (1R) ✨")
                # Full TP at 3R if partial taken
        ...
        if not vol_filter or not trend_filter or not vol_spike:  # Added vol_spike to entry filter
            return

        # Long entry (Loosened RSI: removed <50 cap; Added EMA > close for uptrend; Simplified hist to >0)
        ...
        long_hist_expand = current_hist > 0  # Simplified from > prev and >0 for more signals
        long_trend = current_close > current_ema  # New: Only long in uptrend
        if long_rsi_cross and long_bb_touch and long_macd_cross and long_hist_expand and long_trend:
            ...
            self.risk_distance = current_close - stop_price
            if self.risk_distance > 0:
                risk_amount = self.equity * self.risk_per_trade
                size = risk_amount / self.risk_distance  # Removed int(round) for fractional sizing, more precise
                self.buy(size=size)
                self.entry_price = current_close
                self.stop_price = stop_price
                self.tp_price = current_close + self.rr_ratio * self.risk_distance  # Improved to 3R
                self.partial_taken = False
                self.entry_bar = len(self.data)
                self.is_long = True
                print(f"🌙 Moon Dev LONG Entry at {current_close}, Size: {size}, SL: {stop_price}, TP: {self.tp_price} 🚀")

        # Short entry (Loosened RSI: removed >50 cap; Added EMA < close for downtrend; Simplified hist to <0)
        ...
        short_hist_expand = current_hist < 0  # Simplified from < prev and <0 for more signals
        short_trend = current_close < current_ema  # New: Only short in downtrend
        if short_rsi_cross and short_bb_touch and short_macd_cross and short_hist_expand and short_trend:
            ...
            self.risk_distance = stop_price - current_close
            if self.risk_distance > 0:
                risk_amount = self.equity * self.risk_per_trade
                size = risk_amount / self.risk_distance  # Removed int(round) for fractional sizing
                self.sell(size=size)
                self.entry_price = current_close
                self.stop_price = stop_price
                self.tp_price = current_close - self.rr_ratio * self.risk_distance  # Improved to 3R
