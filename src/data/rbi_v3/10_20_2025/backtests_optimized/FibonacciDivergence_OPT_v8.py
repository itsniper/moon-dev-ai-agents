...
class FibonacciDivergence(Strategy):
    sma_period = 100  # 🌙 Moon Dev: Reduced from 200 to 100 for more responsive trend filter, allowing more trades in trending markets ✨
    rsi_period = 14
    atr_period = 14
    adx_period = 14  # 🌙 Moon Dev: Added ADX for trend strength filter to avoid choppy markets ✨
    adx_threshold = 20  # 🌙 Moon Dev: Threshold set to 20 for sufficient trend without being too restrictive ✨
    vol_period = 20  # 🌙 Moon Dev: Added volume SMA for confirmation filter ✨
    vol_mult = 1.5  # 🌙 Moon Dev: Volume must be 1.5x average for entry to ensure conviction ✨
    strength = 7  # 🌙 Moon Dev: Increased from 5 to 7 for stronger, more reliable pivots ✨
    risk_pct = 0.02  # 🌙 Moon Dev: Increased from 0.01 to 0.02 for higher position exposure to boost returns, balanced with better filters ✨
    min_rsi_div = 3  # 🌙 Moon Dev: Minimum RSI divergence strength to filter weak signals ✨
    min_fib_range_mult = 2  # 🌙 Moon Dev: Minimum swing range must be > mult * ATR to avoid noise trades ✨
    rr_ratio = 3  # 🌙 Moon Dev: Improved RR from 2 to 3 for higher reward potential per trade ✨
    key_fib_levels = [0.0, 0.236, 0.382, 0.5, 0.618, 0.786, 1.0, 1.272, 1.618, 2.618]
    fib_tolerance = 0.03  # 🌙 Moon Dev: Tightened from 0.05 to 0.03 for more precise Fib level hits ✨
    ...
    def init(self):
        ...
        self.adx = self.I(talib.ADX, self.data.High, self.data.Low, self.data.Close, timeperiod=self.adx_period)  # 🌙 Moon Dev: Initialized ADX for regime filter ✨
        self.vol_sma = self.I(talib.SMA, self.data.Volume, timeperiod=self.vol_period)  # 🌙 Moon Dev: Initialized volume SMA for entry confirmation ✨
        self.low_pivots = []
        self.high_pivots = []
                    ...
                    if low_slice[pivot_rel_idx] == low_slice.min():
                        ...
                        if not self.low_pivots or self.low_pivots[-1][0] != pivot_i:
                            ...
                            if len(self.low_pivots) >= 2 and self.data.Close[current_bar] > self.sma[current_bar]:
                                ...
                                # 🌙 Moon Dev: Enhanced divergence check with minimum RSI difference for stronger signals ✨
                                if curr_low < prev_low and curr_rsi > prev_rsi + self.min_rsi_div:
                                    high_start = prev_i + 1
                                    high_end = curr_i + 1
                                    if high_start < high_end and high_end <= current_bar + 1:
                                        ...
                                        # 🌙 Moon Dev: Added minimum range filter to avoid small, noisy swings ✨
                                        if fib_range > self.min_fib_range_mult * self.atr[curr_i]:
                                            ...
                                            # 🌙 Moon Dev: Added ADX and volume filters for better entry quality and regime awareness ✨
                                            if is_at_fib and self.adx[current_bar] > self.adx_threshold and self.data.Volume[current_bar] > self.vol_sma[current_bar] * self.vol_mult:
                                                ...
                                                if atr_val > 0:
                                                    ...
                                                    position_size = risk_amount / stop_distance  # 🌙 Moon Dev: Use float for precise sizing, no int rounding to avoid truncation ✨
                    ...
                    if high_slice[pivot_rel_idx] == high_slice.max():
                        ...
                        if not self.high_pivots or self.high_pivots[-1][0] != pivot_i:
                            ...
                            if len(self.high_pivots) >= 2 and self.data.Close[current_bar] < self.sma[current_bar]:
                                ...
                                # 🌙 Moon Dev: Enhanced divergence check with minimum RSI difference for stronger signals ✨
                                if curr_high > prev_high and curr_rsi < prev_rsi - self.min_rsi_div:
                                    low_start = prev_i + 1
                                    low_end = curr_i + 1
