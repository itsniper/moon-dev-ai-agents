...
class DivergentConvergence(Strategy):
    ...
    lookback = 20  # 🌙 Moon Dev Optimization: Extended lookback to 20 for capturing stronger, more distant divergences in volatile BTC trends
    recent_period = 6  # 🌙 Moon Dev Optimization: Reduced recent period to 6 for focusing on tighter, more immediate low comparisons to improve signal timeliness
    stoch_window = 6  # 🌙 Moon Dev Optimization: Adjusted stochastic window to 6 for faster response to convergence while filtering noise
    risk_pct = 0.02  # 🌙 Moon Dev Optimization: Increased risk per trade to 2% to amplify returns towards 50% target while staying within reasonable risk bounds
    rr = 3.0  # 🌙 Moon Dev Optimization: Boosted RR to 3:1 to capture larger trend moves in BTC, enhancing profitability without overextending
    max_bars = 50  # 🌙 Moon Dev Optimization: Extended max hold time to 50 bars to allow sufficient room for trend development in 15m timeframe
    atr_mult_sl = 1.5  # 🌙 Moon Dev Optimization: Tightened ATR multiplier to 1.5 for more aggressive profit locking, reducing give-back in volatile swings
    buffer_mult = 0.5  # 🌙 Moon Dev Optimization: Tightened buffer to 0.5 ATR for closer initial SL placement, improving risk-reward by minimizing unnecessary risk exposure
    ...
    def init(self):
        ...
        # 🌙 Moon Dev Optimization: Retained volume SMA filter to ensure entries on elevated volume for conviction
        self.vol_sma = self.I(talib.SMA, self.data.Volume, timeperiod=20)
        # 🌙 Moon Dev Optimization: Retained and strengthened ADX filter threshold in entry for stricter trend confirmation
        self.adx = self.I(talib.ADX, self.data.High, self.data.Low, self.data.Close, timeperiod=14)
        # 🌙 Moon Dev Optimization: Added momentum filter using ROC to confirm upward price acceleration before entry
        self.roc = self.I(talib.ROC, self.data.Close, timeperiod=5)
        self.entry_bar = None
    ...
    def next(self):
        ...
        bull_div = (low2 < low1) and (rsi2 > rsi1) and (rsi2 < 35) and (distance <= 5)  # 🌙 Moon Dev Optimization: Tightened RSI threshold to <35 for deeper oversold conditions and extended distance tolerance to <=5 for more flexible but still recent signals
        if bull_div:
            self.div_low = low2
        ...
        stoch_converge = (k_now < 20) and (k_now > k_prev) and (gap_now < gap_prev) and (k_now < d_now)  # 🌙 Moon Dev Optimization: Lowered %K threshold to <20 to target more pronounced oversold convergence in BTC's volatility
        if stoch_converge:
            print(f"🚀 Moon Dev: Stochastic Convergence! %K: {k_now}, %D: {d_now}, Gap narrowed! 🌙")

        # Entry Logic
        if not self.position and self.div_bar is not None and stoch_converge and current_price > self.sma200[-1] and self.data.Volume[-1] > self.vol_sma[-1] and self.adx[-1] > 25 and self.roc[-1] > 0 and self.data.Close[-1] > self.data.Open[-1]:  # 🌙 Moon Dev Optimization: Strengthened ADX to >25 for robust trends, added ROC>0 for momentum confirmation, and bullish candle filter to ensure positive price action
            ...
            if size > 0:
                ...
                self.buy(size=size, tp=tp_price)  # 🌙 Moon Dev Optimization: Retained TP for fixed reward target while using manual trailing for dynamic protection
        ...
        if self.position:
            ...
            # 🌙 Moon Dev Optimization: Enhanced trailing SL using High[-1] for better protection based on actual price highs, updating dynamically
