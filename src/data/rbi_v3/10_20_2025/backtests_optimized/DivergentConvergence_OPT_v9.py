...
class DivergentConvergence(Strategy):
    ...
    lookback = 20
    recent_period = 8  # Tightened recent period for fresher divergence signals 🌙
    stoch_window = 3  # Reduced window for quicker stochastic confirmation ✨
    k_oversold = 15  # Tightened oversold threshold for stronger momentum filter 🚀
    risk_pct = 0.01
    rr = 3  # Increased RR for higher reward potential while maintaining risk management 📈
    max_bars = 15
    atr_mult_sl = 1.5
    buffer_mult = 0.5
    adx_threshold = 20  # Added ADX threshold for trend strength filter to avoid choppy markets 🌙
    vol_mult = 1.0  # Volume multiplier for basic volume confirmation (above average) ✨
    ...
    def init(self):
        ...
        self.ema50 = self.I(talib.EMA, self.data.Close, timeperiod=50)  # Added EMA50 for shorter-term trend confirmation 🚀
        ...
        self.vol_sma = self.I(talib.SMA, self.data.Volume, timeperiod=20)  # Added volume SMA for liquidity/momentum filter 🌙
        self.entry_bar = None
        ...
        self.div_low = None
        self.div_bar = None
        self.risk_per_unit = None  # Added to track risk per unit for trailing calculations ✨
        print("🌙 Moon Dev Backtest Initialized! ✨ Initializing indicators... 🚀")

    def next(self):
        if len(self.data) < 250:  # Increased buffer for new indicators (EMA50 + ADX + vol_sma + lookback) 🌙
            return
        ...
        # RSI Divergence Detection (approximation) - Tightened for better quality setups
        ...
        bull_div = (low2 < low1) and (rsi2 > rsi1) and (rsi2 < 45) and (distance <= 3)  # Tightened RSI <45 and distance <=3 for oversold, recent signals ✨
        if bull_div:
            self.div_low = low2
            self.div_bar = current_bar
            print(f"🌙 Moon Dev: Bullish RSI Divergence detected! Price low: {low2}, RSI: {rsi2} ✨ Distance: {distance} bars")

        # Stochastic Convergence - Tightened for stronger convergence signal
        k_now = self.slowk[-1]
        d_now = self.slowd[-1]
        ...
        stoch_converge = (k_now < self.k_oversold) and (k_now > k_prev) and (gap_now < gap_prev) and (k_now < d_now)  # Used k_oversold for deeper oversold filter 🚀
        if stoch_converge:
            print(f"🚀 Moon Dev: Stochastic Convergence! %K: {k_now}, %D: {d_now}, Gap narrowed! 🌙")

        # Entry Logic - Added multi-filters: EMA50 trend, volume, ADX for higher quality entries 📊
        if not self.position and self.div_bar is not None and stoch_converge and current_price > self.ema50[-1] and self.ema50[-1] > self.sma200[-1] and self.data.Volume[-1] > self.vol_sma[-1] * self.vol_mult and self.adx[-1] > self.adx_threshold:
            ...
            if size > 0:
                ...
                self.risk_per_unit = risk_per_unit  # Saved for trailing stop calculations ✨
                self.div_bar = None
                self.div_low = None
        ...
        if self.position:
            ...
            # Added trailing stop: Activate after 1R profit, trail at ATR * 1.5 below current price for better profit locking 🚀
            if self.risk_per_unit is not None:
                profit_r = (current_price - self.entry_price) / self.risk_per_unit
                if profit_r > 1.0:
                    new_sl = current_price - self.atr[-1] * self.atr_mult_sl
                    if new_sl > self.position.sl:
                        self.position.sl = new_sl
                        print(f"🌙 Moon Dev: Trailing SL updated to {new_sl} (Profit R: {profit_r:.2f}) 🚀")
