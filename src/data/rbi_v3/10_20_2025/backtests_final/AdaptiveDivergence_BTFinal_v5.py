...
class AdaptiveDivergence(Strategy):
    ...
    def init(self):
        ...
        print(f"🌙 AdaptiveDivergence initialized! 🚀 Initial cash: {self._broker.cash:,} ✨")
    ...
    def next(self):
        ...
        # Entry logic if no position
            ...
            if (bullish_div and self.current_mom > self.mom_long_thresh and mom_rising and above_ema):
                equity = self._broker.cash + self.position.size * self.data.Close[-1]
                risk_amount = equity * self.risk_per_trade
                sl_distance = self.sl_mult * self.atr[-1]
                raw_size = risk_amount / sl_distance
                cash_size = self._broker.cash / self.data.Close[-1]
