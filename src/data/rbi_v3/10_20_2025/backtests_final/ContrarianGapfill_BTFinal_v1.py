...
class ContrarianGapfill(Strategy):
    ...
    def next(self):
        ...
        if gap_pct > self.gap_threshold:
            print(f"🌙 Moon Dev Gap Detected: {gap_pct*100:.2f}% down | RSI: {self.rsi[-1]:.1f} | Vol: {self.data.Volume[-1]:.0f} vs SMA: {self.volume_sma[-1]:.0f} ✨")
        ...
        if (gap_pct > self.gap_threshold and
            ...
            print(f"   Calculated size: {size} units | Equity: {equity:.2f} | Risk: {risk_amount:.2f}")
...
stats = bt.run()
print(stats)
print(f"🌙 Moon Dev Backtest Complete! Total Trades: {stats.get('Total Trades', 0)} | Return: {stats.get('Return [%]', 'NaN'):.2f}% ✨")