...
# Convert datetime and set index properly
...
class ContrarianGapfill(Strategy):
    ...
    def next(self):
        ...
        if gap_pct > 0.02:
            vol_ratio = self.data.Volume[-1] / self.volume_sma[-1] if not pd.isna(self.volume_sma[-1]) and self.volume_sma[-1] != 0 else 0
        ...
        if (gap_pct > self.gap_threshold and
            ...
            not pd.isna(self.volume_sma[-1]) and self.data.Volume[-1] > self.volume_sma[-1] * self.volume_multiplier):
